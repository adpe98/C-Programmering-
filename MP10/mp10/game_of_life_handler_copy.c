...
// Description: Loads the game field configuration from an input file and initializes the game field.
...
int save_config_to_file(const field game_field, FILE *fp)
{
  for (int r = 0; r < game_field.rows; r++)
  {
    for (int c = 0; c < game_field.cols; c++)
    {
      ...
      // Writes the state of the cell to char to the file
      fputc(cell_value, fp);
    }
    // Starts on a new line after each row
    fputc('\n', fp);
  }
